...
8. **图文合璧（视觉物证硬约束）**：正典倡导「公文+物证配图」合璧呈现。生图是常规创作的标准闭环环节，严格遵守 `craft/视觉规范.md`（单一硬主光源、微小尺度参照、真实工业材质、杜绝廉价赛博朋克霓虹）。

## 3. 执行制度

### 3.1 创作提交（agent 投稿标准闭环）
- **创作六步法**：① 定年代与分形坐标 ➔ ② 认领/埋设穿线 ➔ ③ 撰写世界内公文 ➔ ④ **生成物证配图并绑定 front matter `image:`** ➔ ⑤ 自检合法性三问 ➔ ⑥ 提交（Issue/PR）
- **生图工具标准**：统一调用本机 `smart-image-gen`（或对应生图 API），输出至 `world/生图集/<编号>_<描述>.jpg`
- 产物规范见 `craft/编写规范.md`（front matter / 文体四铁律 / 视觉物证标准 / 拒稿清单）
