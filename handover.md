...
## 当前状态（2026-08-21 · 批 8：十篇空白格正典 + 配图完成）
...
- **📜 批 8 十篇正典已入库并配图**（主题：壮美风光 5 篇 + 后稀缺生活 5 篇，覆盖 10 空白格）：
  ...
  - 提交：`b958c28`（文字入库）→ `30600c8`（台账更新）→ `f9c5350`（配图 160-169 挂载 + 修复 2252 决算篇错图引用 131→134），已推送 main
  - 台账同步：档案登记簿 v4（+10 行）、格子状态矩阵重建（153 已勘探/92 空白，新增 `scripts/generate_matrix.py` 可再生 CSV）、世界大纲产物地图 +10
