...
## 当前状态（2026-08-21 · 批 8 收官归档：十篇空白格正典 + 配图 + 档案页同步完成）

- **📜 批 8 十篇正典已入库并配图**（主题：壮美风光 5 篇 + 后稀缺生活 5 篇，覆盖 10 空白格）：
  - 壮美风光：木星大红斑测深 GS-2048-06（知识×竞赛×④）/ 谷神星亮斑卤水生态 GS-2077-02（生态×丰裕×③）/ 「目的地想象」征集公告 GS-2158-02（文化×启航×⑤，比邻星禁区合规范本）/ 月面观光健康监测 GS-2603-01（人×双星系×②）/ 主带观光条例 GS-2801-01（社会×双星系×③）
  - 后稀缺生活：盈余时间规划 GS-2072-02（文化×丰裕×①）/ 聚光熔炼前哨休整 GS-2076-05（人×丰裕×③）/ 主带前哨自治公约 GS-2077-03（社会×丰裕×③）/ 意义资产清册 GS-2115-01（经济×离心×①）/ 中继港留守评估 GS-2156-01（人×启航×③）
  - **校验：`check_submission.py --all` → 170/170 合规、异常 0；archive_id 无冲突、10 格均为空白格首开**
  - 提交链：`b958c28`（文字）→ `30600c8`（台账）→ `f9c5350`（配图+2252修复）→ `d1d1a4e`（handover）→ `0150782`（首页）→ `15d53ff`（文档同步）→ `bb7b32a`（档案页数据 150→170）→ `0e71f4c`（promotion 副本同步）→ `c00dd82`（配图圆点修复）
  - 台账同步：档案登记簿 v4（+10 行）、格子状态矩阵重建（153 已勘探/92 空白，`scripts/generate_matrix.py` 可再生）、世界大纲产物地图 +10
  - **注意**：本批 10 张配图右下角带「AI生成」水印（模型今日升级强制加 AI 标识，批 7 的 150-159 无水印）；已按用户确认接受入库
- **🗄 档案舱页面（heatmap_3d.html）已同步并修复**：
  - 内嵌 DATA 重建为 170 篇/153 格（新增 `scripts/rebuild_heatmap_data.py` 从 front matter 再生，解决手写过时根因）
  - 修复配图圆点标识消失 bug（重建脚本漏 `full_text` 字段 → 圆点 151 个恢复）
  - promotion 旧副本（28 篇死数据）同步为最新，消除误导源
  - 线上验证：统计 170/92/153、圆点 151，均正确
